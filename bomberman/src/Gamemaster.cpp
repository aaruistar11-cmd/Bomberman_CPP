...
// Gestisce il responsive e la partita in generale
...
// Inizia a disegnare il menu principale
...
// Inizializzazione della partita vera e propria
void Gamemaster::startpartita() {
    ...
    while (stato==STATO_PARTITA) {
        // Aggiorna timer e dimensione finestra
        responsive();
        ...
        // Controllo delle bombe piazzate
        bnode currB = bhead;
        while(currB != nullptr){
...
// Svuota completamente le liste per non lasciare garbage in memoria
...
// Loop di stato (Menu -> Partita -> Classifica)
void Gamemaster::run() {
    getmaxyx(stdscr, oldH, oldW);
