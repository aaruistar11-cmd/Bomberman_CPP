...
Mappa::Mappa(int livello, unsigned int semeBase) {
    ...
    // Aggiungo il livello al random così le mappe sono diverse
...
// Inserimento dei blocchi nella mappa
void Mappa::inizializzaGriglia() {
    // Ciclo per piazzare muri e casse sparsi
    int densitaCasse = 0;
    int densitaSolidi = 0;
...
// Stampa effettiva a schermo
void Mappa::disegnaLayout(int punti, int tempo, int vite) {
    erase();
    
    // Misure della grafica attorno alla griglia
    ...
    // Traccia le linee per il box grande
    ...
    // Stampa i muri, le casse vuote e il pavimento
...
// Evidenzia le entità sulla griglia stampandole "sopra"
...
// Ritorna il carattere presente alle coordinate date
